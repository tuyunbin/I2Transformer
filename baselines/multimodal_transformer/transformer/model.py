...
class IEE(nn.Module):
    def __init__(self, config):
        super(IEE, self).__init__()

        self.gate_r1 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_i1 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_s1 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_r2 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_i2 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_s2 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_r3 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_i3 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )
        self.gate_s3 = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Dropout(config.hidden_dropout_prob),
        )


        self.res_intermediate = BertIntermediate(config)
        self.res_output = BertOutput(config)

        self.i3d_intermediate = BertIntermediate(config)
        self.i3d_output = BertOutput(config)

        self.sub_intermediate = BertIntermediate(config)
        self.sub_output = BertOutput(config)

    def forward(self, res_embeddings, i3d_embeddings, sub_embeddings):
        """
        Args:
            hidden_states: (N, L, D)
            attention_mask: (N, L)
        ...
        gate_r = torch.sigmoid(self.gate_r1(res_embeddings) + self.gate_i1(i3d_embeddings) + self.gate_s1(sub_embeddings))
        gate_i = torch.sigmoid(self.gate_r2(res_embeddings) + self.gate_i2(i3d_embeddings) + self.gate_s2(sub_embeddings))
        gate_s = torch.sigmoid(self.gate_r3(res_embeddings) + self.gate_i3(i3d_embeddings) + self.gate_s3(sub_embeddings))

        res_gate = gate_r * res_embeddings
        i3d_gate = gate_i * i3d_embeddings
        sub_gate = gate_s * sub_embeddings

        res_intermediate = self.res_intermediate(res_gate)
        res_output = self.res_output(res_intermediate, res_gate)

        i3d_intermediate = self.i3d_intermediate(i3d_gate)
        i3d_output = self.i3d_output(i3d_intermediate, i3d_gate)

        sub_intermediate = self.sub_intermediate(sub_gate)
        sub_output = self.sub_output(sub_intermediate, sub_gate)

        return res_output, i3d_output, sub_output
...
class IAE(nn.Module):
    ...
    def forward(self, reps, reps_mask):
        """
        Args:
            hidden_states: (N, L, D)
            attention_mask: (N, L)
        ...
        reps_mask = (1 - reps_mask.unsqueeze(1)) * -10000.  # (N, 1, Lvideo)

        reps_graph = reps_graph + reps_mask
...
class BertEmbeddingsWithVideo(nn.Module):
    ...
    def __init__(self, config, add_postion_embeddings=True):
        ...
        self.res_video_embeddings = nn.Sequential(
            BertLayerNorm(config.res_video_feature_size, eps=config.layer_norm_eps),
            nn.Dropout(config.hidden_dropout_prob),
            nn.Linear(config.res_video_feature_size, config.hidden_size),
            nn.ReLU(True),
            BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps),
        )

        self.i3d_video_embeddings = nn.Sequential(
            BertLayerNorm(config.i3d_video_feature_size, eps=config.layer_norm_eps),
            nn.Dropout(config.hidden_dropout_prob),
            nn.Linear(config.i3d_video_feature_size, config.hidden_size),
            nn.ReLU(True),
            BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps),
        )

        self.sub_layer = nn.ModuleList([IAE(config) for _ in range(2)])
        self.res_layer = nn.ModuleList([IAE(config) for _ in range(2)])
        self.i3d_layer = nn.ModuleList([IAE(config) for _ in range(2)])
        self.gate_layer = nn.ModuleList([IEE(config) for _ in range(2)])
    ...
    def forward(self, input_ids, res_video_features, res_vid_mask, i3d_video_features, i3d_vid_mask, token_type_ids, ctx_mask):
        """
        Args:
            input_ids: (N, L)
        ...
        res_video_embeddings = self.res_video_embeddings(res_video_features)
        i3d_video_embeddings = self.i3d_video_embeddings(i3d_video_features)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        # GCN to model relation

        for layer_idx, layer_module in enumerate(self.sub_layer):
            words_embeddings = layer_module(words_embeddings, ctx_mask)

        for layer_idx, layer_module in enumerate(self.res_layer):
            res_video_embeddings = layer_module(res_video_embeddings, res_vid_mask)

        for layer_idx, layer_module in enumerate(self.i3d_layer):
            i3d_video_embeddings = layer_module(i3d_video_embeddings, i3d_vid_mask)

        for layer_idx, layer_module in enumerate(self.gate_layer):
            res_video_embeddings, i3d_video_embeddings, words_embeddings = layer_module(res_video_embeddings, i3d_video_embeddings, words_embeddings)

        # sum fusion
        # embeddings = words_embeddings + res_video_embeddings + i3d_video_embeddings + token_type_embeddings

        # con fusion
        con_embeddings = torch.cat((words_embeddings, res_video_embeddings, i3d_video_embeddings, token_type_embeddings), -1)
        con_embeddings = self.con_fc(con_embeddings)
        media_embeddings = self.ff1(con_embeddings)
        embeddings = self.ff2(media_embeddings, con_embeddings)
...
class MMT(nn.Module):
    ...
    def encode(self, ctx_input_ids, ctx_input_mask, ctx_token_type_ids, res_video_feature, res_video_mask, i3d_video_feature, i3d_video_mask):
        """
        Args:
        ...
        """
        bat, len = ctx_input_mask.shape[0:]
        res_video_valid_mask = torch.FloatTensor(bat, len).zero_().cuda()
        res_video_valid_mask[:, :20] = res_video_mask
        i3d_video_valid_mask = torch.FloatTensor(bat, len).zero_().cuda()
        i3d_video_valid_mask[:, :20] = i3d_video_mask
        ctx_embeddings = self.embeddings(ctx_input_ids, res_video_feature, res_video_valid_mask, i3d_video_feature, i3d_video_valid_mask, ctx_token_type_ids, ctx_input_mask)  # (N, Lctx, D)
    ...
    def forward(self, ctx_input_ids, ctx_input_mask, ctx_token_type_ids, res_video_feature, res_video_mask, i3d_video_feature, i3d_video_mask,
        ...
        encoder_outputs = self.encode(ctx_input_ids, ctx_input_mask, ctx_token_type_ids, res_video_feature, res_video_mask, i3d_video_feature, i3d_video_mask)  # (N, Lv, D)
