...
class TVCaptionDataset(Dataset):
    ...
    def __init__(self, ctx_mode, data_path, sub_meta_path, res_vid_h5_path_or_handler, i3d_vid_h5_path_or_handler, word2idx_path,
        ...
        if self.use_video:
            if isinstance(res_vid_h5_path_or_handler, h5py.File) and isinstance(i3d_vid_h5_path_or_handler, h5py.File):
                self.res_vid_h5 = res_vid_h5_path_or_handler
                self.i3d_vid_h5 = i3d_vid_h5_path_or_handler
            else:
                self.res_vid_h5 = h5py.File(res_vid_h5_path_or_handler, "r", driver=h5driver)
                self.i3d_vid_h5 = h5py.File(i3d_vid_h5_path_or_handler, "r", driver=h5driver)
                # self.res_vid_h5 = res_vid_h5_path_or_handler
                # self.i3d_vid_h5 = i3d_vid_h5_path_or_handler
    ...
    def convert_example_to_features(self, example):
        ...
        """
        ctx_input_ids, ctx_input_mask, ctx_token_type_ids, res_video_feature, res_video_mask, i3d_video_feature, i3d_video_mask = self.get_context(example)
        if self.is_eval:
        ...
        data = dict(
            ...
            res_video_feature=res_video_feature.astype(np.float32),
            res_video_mask=np.array(res_video_mask).astype(np.float32),
            i3d_video_feature=i3d_video_feature.astype(np.float32),
            i3d_video_mask=np.array(i3d_video_mask).astype(np.float32)
        )
        meta = example
        return data, meta
    ...
    def get_context(self, example):
        ...
        if self.use_video:
            res_video_feature = self.res_vid_h5[vid_name]  # (L, D)
            res_video_feat, res_video_tokens, res_video_mask = self._load_indexed_video_feature(res_video_feature, clip_st_ed)
            i3d_video_feature = self.i3d_vid_h5[vid_name]  # (L, D)
            i3d_video_feat, i3d_video_tokens, i3d_video_mask = self._load_indexed_video_feature(i3d_video_feature,
                                                                                                clip_st_ed)
        else:
            res_video_feat, res_video_tokens, res_video_mask = np.zeros((2+self.max_sub_len, 2)), [0, 0], [0, 0]  # fake inputs
            i3d_video_feat, i3d_video_tokens, i3d_video_mask = np.zeros((2 + self.max_sub_len, 2)), [0, 0], [0,
                                                                                                             0]  # fake inputs
        if self.use_sub:
        ...
        ctx_input_tokens = res_video_tokens + sub_tokens

        ctx_input_ids = [self.word2idx.get(t, self.word2idx[self.UNK_TOKEN]) for t in ctx_input_tokens]
        ctx_input_mask = res_video_mask + sub_mask
        ctx_token_type_ids = [0] * self.max_v_len + [1] * self.max_sub_len
        return ctx_input_ids, ctx_input_mask, ctx_token_type_ids, res_video_feat, res_video_mask, i3d_video_feat, i3d_video_mask
